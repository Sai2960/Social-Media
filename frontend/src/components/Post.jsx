      ...
      <div className='w-[90%]   flex flex-col gap-[10px] bg-white items-center shadow-2xl shadow-[#00000058] rounded-2xl pb-[20px]'>
        <div className='w-full h-[80px] flex justify-between items-center px-[10px]'>
          <div className='flex justify-center items-center md:gap-[20px] gap-[10px]' onClick={()=>navigate(`/profile/${post.author?.userName}`)}>
            {/* FIX 1a: Added h-full to author profile image */}
          ...
          {/* FIX 2: Use .toString() comparison to avoid loose equality failure */}
          {userData._id?.toString() !== post.author._id?.toString() && <FollowButton tailwind={'px-[10px] min-w-[60px] md:min-w-[100px] py-[5px] h-[30px] md:h-[40px] bg-[black] text-white rounded-2xl text-[14px] md:text-[16px]'} targetUserId={post.author._id}/>}
        ...
        {/* FIX 3: Fixed caption layout — semibold username + caption side by side, aligned to start */}
        {post.caption && <div className='w-full px-[20px] flex justify-start items-start gap-[8px]'>
        ...
        {showComment &&
          <div className='w-full  flex flex-col gap-[30px] pb-[20px]'>
            ...
            {/* FIX 1b: Added h-full to comment input section author image */}
            <div className='w-[40px] h-[40px] md:w-[60px] md:h-[60px] border-2 border-black rounded-full cursor-pointer overflow-hidden'>
              <img src={post.author?.profileImage || dp} alt="" className='w-full h-full object-cover' />
            </div>
  ...
  <div key={index} className='w-full px-[20px] py-[20px]  flex items-center gap-[20px] border-b-2 border-b-gray-200'>
    {/* FIX 1c: Added h-full to commenter profile image */}
    <div className='w-[40px] h-[40px] md:w-[60px] md:h-[60px] border-2 border-black rounded-full cursor-pointer overflow-hidden'>
