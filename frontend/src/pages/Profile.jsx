...
function Profile() {
  ...
  // ✅ Use postData from Redux for saved posts filter only
  ...
  const isOwnProfile = profileData?._id?.toString() === userData?._id?.toString();

  // ✅ FIX: Filter out posts where author is not populated (unpopulated = string ID, not object)
const safeProfilePosts = profileData?.posts || [];
    ...
    <div className="w-full min-h-screen bg-black">
      ...
      <div className="w-full min-h-[100vh] flex justify-center">
        <div className="w-full max-w-[900px] flex flex-col items-center rounded-t-[30px] bg-white relative gap-[20px] pt-[30px] pb-[100px]">
          ...
          {/* Posts/Saved Toggle - only for own profile */}
          {isOwnProfile && (
          ...
          {/* ✅ Own profile — Posts tab: use safeProfilePosts (author must be populated object) */}
          {isOwnProfile && postType == "posts" && (
            safeProfilePosts.length > 0
              ? safeProfilePosts.map((post, index) => (
                  <Post key={index} post={post} />
                ))
              : <div className="text-gray-400 text-[16px] mt-[20px]">No posts yet</div>
          )}

          {/* ✅ Own profile — Saved tab: still use Redux postData */}
          {isOwnProfile && postType == "saved" && (
            postData.filter(post => userData?.saved?.includes(post._id)).length > 0
              ? postData
                  .filter(post => userData?.saved?.includes(post._id))
                  .map((post, index) => <Post key={index} post={post} />)
              : <div className="text-gray-400 text-[16px] mt-[20px]">No saved posts yet</div>
          )}

          {/* ✅ Other user's profile: use safeProfilePosts (author must be populated object) */}
          {!isOwnProfile && (
            safeProfilePosts.length > 0
              ? safeProfilePosts.map((post, index) => (
                  <Post key={index} post={post} />
